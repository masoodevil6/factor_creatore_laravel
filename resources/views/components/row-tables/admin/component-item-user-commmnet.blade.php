<tr   title="@if($commentSeen==1){{"دیده شده"}}@else {{"دیده نشده"}}@endif">
    ...
    <td class="font-size-12">
        <x-fields.component-input-check-box
                title-fa='وضعیت'
                title-en='status'
                :url='route("admin.users.comment.status" , $commentId)'
                :value='$commentStatus'/>
    </td>
    <td class="font-size-12">
        <x-fields.component-input-check-box
                title-fa='تاییدیه'
                title-en='approved'
                :url='route("admin.users.comment.approved" , $commentId)'
    ...
    <td class="text-left font-size-12 py-2">
        ...
        <x-fields.component-button
                btn-type='delete'
                :url='route("admin.users.comment.destroy" , $commentId)'/>

        <x-fields.component-button
                btn-type='edit'
                :url='route("admin.users.comment.edit" , $commentId)'/>
        ...
        @if($commentParent == null)
            <x-fields.component-button
                    ...
                    title='پاسخ'
                    :url='route("admin.users.comment.adminAnswer" , $commentId)'/>
        @endif
