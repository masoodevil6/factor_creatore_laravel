    ...
    <section class="row  mx-0 border border-dark rounded py-2 ">
        ...
        <section class="col-12 col-lg-6">
            ...
            @if(!empty($factor->logo_name))
                ...
                <img src="{{route("customer.images-factor.get-template-logo-image")}}" class="d-block m-auto" height="100" alt="تصویر لوگو" style="max-width: 100%">
                <form method="post" action="{{route("customer.images-factor.delete-template-logo-image")}}" class="mt-1 d-block">
                    @csrf
        ...
        <form method="post" action="{{route("customer.images-factor.upload-template-logo-image")}}" enctype="multipart/form-data" class="col-12 col-lg-6 mt-5 mt-lg-0">
            @csrf
            <label class="form-label mb-1" for="user-logo">انتخاب فایل لوگو</label>
            <input name="logo_name" type="file" class="form-control line-height-30 p-0" id="user-logo" />
            <x-input-errors field="logo_name"/>
